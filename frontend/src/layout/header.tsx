...
const Header = ({ onResetChat }: HeaderProps) => {
  ...
  return (
    <header className="w-full h-13 bg-[#BADDFE] backdrop-blur border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 relative">
      <button onClick={onResetChat} className="active:scale-[0.98] transition">
      ...
      <div className="relative" onMouseEnter={() => setShowLanguageMenu(true)} onMouseLeave={() => setShowLanguageMenu(false)}>
        <div className="inline-flex items-center gap-1 cursor-pointer">
          ...
          <svg className="w-3.5 h-3.5 opacity-70 text-[#6C6B6B]" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        {showLanguageMenu && (
          <div className="absolute right-0 top-full z-50 rounded-md border border-slate-200 bg-white shadow-lg w-28" role="menu">
            <button onClick={() => handleLanguageChange('한국어')} className="w-full text-left px-3 py-2 text-sm hover:bg-[#E9F4FF]" role="menuitem">
              한국어
            </button>
            <button onClick={() => handleLanguageChange('English')} className="w-full text-left px-3 py-2 text-sm hover:bg-[#E9F4FF]" role="menuitem">
              English
            </button>
          </div>
        )}
